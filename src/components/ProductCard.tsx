...
export default function ProductCard({ product, onAddToCart, onViewDetails }: ProductCardProps) {
  // Usa la primera imagen de la nueva lista, o la imagen antigua como respaldo
  const displayImage = (product.image_urls && product.image_urls[0]) || product.image_url;
  ...
  return (
    <div className="card-hover bg-white rounded-2xl overflow-hidden shadow-md border border-rose-100/50 fade-in">
      <div className="relative overflow-hidden h-72 bg-gradient-to-br from-rose-50 to-amber-50">
        <img
          src={displayImage}
          alt={product.name} // Corregido a name
      ...
      <div className="p-6">
        <h3 className="text-xl font-serif font-bold text-gray-800 mb-2">{product.name}</h3> {/* Corregido a name */}
